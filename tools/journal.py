...
def journal_init(b_path):
    logger = logging.getLogger(__name__)
    if not os.path.exists(b_path):
        logger.warning(
            f"Journal base path does not exist: {b_path}, \n Creating now..."
        )
    Path(b_path).mkdir(parents=True, exist_ok=True)


def write_journal(content: str):
    base_path = "./workspaces/Yui/world"
    journal_init(b_path=base_path)
    today = str(date.today())
    f_path = base_path + today + ".txt"
...
class Journal:
    def __init__(self, base_path="./workspaces/Yui/world"):
        ...
        if not os.path.exists(base_path):
            self.logger.warning(
                f"Journal base path does not exist: {base_path}, \n Creating now..."
            )
